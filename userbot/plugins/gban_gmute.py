...
async def gspider(fridayuserbot):
    lol = fridayuserbot
    ...
    me = await fridayuserbot.client.get_me()
    await friday.edit(f"Global Ban Is Coming ! Wait And Watch You Nigga")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await fridayuserbot.get_chat()
    a = b = 0
    if fridayuserbot.is_private:
        user = fridayuserbot.chat
        reason = fridayuserbot.pattern_match.group(1)
    else:
        fridayuserbot.chat.title
    try:
        user, reason = await get_full_user(fridayuserbot)
    except:
        pass
    try:
        if not reason:
    ...
    if user:
        ...
        try:
            from fridayuserbot.modules.sql_helper.gmute_sql import gmute
        except:
            pass
        try:
            await fridayuserbot.client(BlockRequest(user))
        except:
            pass
        testfridayuserbot = [
            d.entity.id
            for d in await fridayuserbot.client.get_dialogs()
            if (d.is_group or d.is_channel)
        ]
        for i in testfridayuserbot:
            try:
                await fridayuserbot.client.edit_permissions(i, user, view_messages=False)
                a += 1
...
async def gspider(fridayuserbot):
    ...
    if user:
        ...
        try:
            from fridayuserbot.modules.sql_helper.gmute_sql import ungmute
        except:
            pass
        try:
            await fridayuserbot.client(UnblockRequest(user))
        except:
            pass
        testfridayuserbot = [
            d.entity.id
        ...
        for i in testfridayuserbot:
            try:
                await fridayuserbot.client.edit_permissions(i, user, send_messages=True)
                a += 1
...
async def handler(rkG): 
   if rkG.user_joined or rkG.user_added:      
       try:       	
         from fridayuserbot.modules.sql_helper.gmute_sql import is_gmuted
